...
* **v1.2.0**:
* **送信ガードレールの共通化（`senders.py`）**:
* Discord（Simple / Embed）や Bluesky などの各配信先において、文字数オーバーによる送信エラー（配信落ち）を確実に防止するため、スライシングによるガードレール処理を共通実装。
* **地震情報パーサの刷新（`quake_parser.py`）**:
* 戻り値を辞書形式（タイトル・説明文・カラー）に統一し、呼び出し側での動的なタイトル・カラー制御に対応。
* 津波情報、震度速報、遠地地震、南海トラフ関連などの解説情報について、重要度や緊急度（赤・オレンジ・青）に応じた埋め込みカラーの自動切り替えを実装。
* **気象・警報情報の安定化**:
* 「北海道」などの広域指定時にデータ量が膨れ上がってDiscordの制限に引っかかるケースを事前に検知・弾くガード処理（1900文字制限）を追加。
* 市町村からローカルコード（十勝中部など）へのマッピングや、フォールバック検索のロジックを整理・最適化。
* **出典表記の統一**:
...
* bot類その他細かい手直し。
* **v1.3.0**:
* **火山情報パーサの新設（`volcano_parser.py`）**: 気象庁の火山噴火警報・予報JSONのパース処理に対応。「防災対応」や「対象市町村」リストの柔軟な抽出・整形を行い、警戒レベルに応じたカラーマップによる自動色分けを実装。
* **URL連携・フォールバックの堅牢化**: 火山情報等の通知において、新旧URLキー（`jma_web_url` / `jma_url`）を柔軟に捉える仕組みを導入し、データソースの変化に対応。
* **マルチプラットフォーム送信モジュールの制御整理（`senders.py`）**: `is_short` 引数および `mode="limit"` による、柔軟なテキスト短縮・制限機能を統合。
* **Bluesky対応の安定化（AT Protocol）**: プロトコル仕様の300バイト制限（UTF-8）に合わせた厳密な文字数切り詰め処理と、URLの自動リンク化（Facets）実装の安定性を向上。