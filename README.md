...
4. **対話型チャットボットの利用**
* 必要に応じて `bot.bat` を起動することで、チャットからの対話型情報照会が利用可能になります（DiscordとMatrixのみ対応）。
* **対応しているチャットの例**:
* 天気情報: `○○の天気は` または `天気/○○`
* 気象情報: `○○の気象情報は` または `気象/○○`
...
* [詳細な更新履歴 (ChangeLog)](https://mustangtis.wjg.jp/J-Sentinel/)
...
* **v1.0.0**: 気象警報インジェスト・パーサの強化、危険レベルの動的連携とグループ化対応。
* **v0.x系**: コアインジェスト基盤の構築、マルチプラットフォーム配信および対話型ボット（Discord / Matrix）の統合。
