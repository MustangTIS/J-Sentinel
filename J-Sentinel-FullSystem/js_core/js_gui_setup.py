...
class JSentinelSetupApp:
    ...
    def __init__(self, root):
        ...
        self.root.geometry("480x700")  # 火山タスク・CSVボタン追加に伴い高さを700に調整
    ...
    def load_config(self) -> dict:
        ...
        if CONFIG_PATH.exists():
            try:
                ...
                # 既存のconfigに新しいタスクがない場合のフォールバック（安全対策）
                tasks = loaded.setdefault("tasks", {})
                if "volcano" not in tasks:
                    tasks["volcano"] = {
                        "enabled": True,
                        "script": "fetch_volcano.py",
                    }
                if "forecast" not in tasks:
                    tasks["forecast"] = {
                        "enabled": True,
...
'''
    ...
    def create_widgets(self):
        ...
        # ★追加: 火山情報タスク
        self.task_volcano_var = tk.BooleanVar()
        ttk.Checkbutton(
            task_frame,
            text="火山情報タスク (fetch_volcano.py)",
            variable=self.task_volcano_var,
        ).pack(anchor=tk.W, pady=2)
        ...
        # ボタンが増えたため 1行目・2行目に分けてすっきり配置
        csv_row1 = ttk.Frame(csv_frame)
        csv_row1.pack(fill=tk.X, pady=2)
        ...
        csv_row2 = ttk.Frame(csv_frame)
        csv_row2.pack(fill=tk.X, pady=2)

        # ★追加: volcanosorter.csv ボタン
        ttk.Button(
            csv_row2,
            text="volcanosorter.csv を開く",
            command=lambda: self.open_csv("volcanosorter.csv"),
        ).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
